...
public class ContactUpdateFrameAtHome extends JFrame {
	...
	public static void showContactUpdateFrame(Component pc,int index) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					ContactUpdateFrameAtHome frame = new ContactUpdateFrameAtHome(pc, index);
	 ...
	 * Create the frame.
	 */
	public ContactUpdateFrameAtHome(Component pc,int index) {
		this.index = index;
	...
	private void updateContact() {
		...
		if (result==1) {
			...
			JOptionPane.showMessageDialog(parentComponent, "성공했습니다.");
