...
//객체를 생성하지 못하도록 작성. -> 모든 메서드는 public static으로.
public class FileUtil { //파일 일고 쓰는걸 따로 만드는거. dao에서 만들어도 되긴 하는데 너무 지저분해지니까!
	public static final String DATA_DIR = "data"; //데이터 파일을 저장할 폴더 이름
	public static final String DATA_FILE = "contacts.dat"; //데이터 파일 이름

	//private 생성자 -> 다른 클래스에서 생성자를 호출할 수 없음(객체생성불가)
	private FileUtil() {}
	
	/**
	 * 연락처 데이터 파일을 저장하는 폴더가 생성되어 있지 않으면,
	 * 폴더를 새로 생성하고FIle  타입 객체를 리턴
	...
	public static List<Contact> readDataFromFile(){
		...
		File file = new File(DATA_DIR, DATA_FILE); //FILE이 없어서 오류 발생 가능. FileNotFound
